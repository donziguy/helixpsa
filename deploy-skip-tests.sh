#!/bin/bash
# HelixPSA deploy script — build, push, deploy to Docker-Server (skip tests)
set -e
...
echo "🧬 HelixPSA Deploy (Skip Tests)"
echo "=============================="
...
git add -A
git diff --cached --quiet || git commit -m "feat: add client portal - external view for clients to submit/track tickets ($(date +%Y-%m-%d-%H%M))"
